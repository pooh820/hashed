...
if ($request == 1) {
    ...
    $result = $con->query("SELECT COUNT(*) AS allcount FROM users where username != 'leo'");
    if ($result) {
    ...
    $result = $con->query("SELECT COUNT(*) AS allcount FROM users WHERE 1 " . $searchQuery . "AND username != 'leo'");
    if ($result) {
        $records = $result->fetch_assoc();
    ...
    // Fetch records
    $empRecords = $con->query("SELECT * FROM users WHERE 1 " . $searchQuery . "AND username != 'leo' LIMIT " . $row . ", " . $rowperpage);
    $data = array();
...
if ($request == 3) {
    ...
    if ($record && $record->num_rows > 0) {
        $con->query("UPDATE users SET password='".$hashed_password."' WHERE id=" . $id);
        echo 1;
        exit;
    } else {
        echo 0;
...
if ($request == 6) {
    ...
    if ($username != '') {
        if ($con->query("INSERT INTO users (username, password) VALUES ('" . $username . "', '" . $hashed_password . "')")) {
